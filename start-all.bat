@echo off
REM Tempolink - tek komutla tum servisleri baslatir (Redis + server + client).
...
echo ============================================
echo   Tempolink baslatiliyor...
echo ============================================

echo [1/3] Redis baslatiliyor...
start "Tempolink - Redis" cmd /k call "%~dp0redis-portable\start-redis.bat"

timeout /t 2 /nobreak >nul

echo [2/3] Sunucu (server) baslatiliyor...
start "Tempolink - Server" cmd /k call "%~dp0server\start-server.bat"

timeout /t 2 /nobreak >nul

echo [3/3] Istemci (client) baslatiliyor...
start "Tempolink - Client" cmd /k call "%~dp0client\start-client.bat"
